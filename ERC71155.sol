...
contract ERC1155 is IERC1155{
    ...
    function safeTransferFrom(
        ...
        bytes calldata data ) external {
            ...
            require(to!= address(0),"to= 0x00.. address");
    ...
    function supportsInterface(bytes4 interfaceId)
        ...
        returns (bool)
    {
        return // ID for ERC165
               interfaceId == 0x01ffc9a7 
               // ID for ERC1155
            || interfaceId == 0xd9b67a26 
            // ID for ERC1155MetadataURI
    ...
    // Internal functions that is called by inside and inherited contract
     ...
     function _batchMint(address to,uint256[] calldata ids,
    ...
    function _burn(address from ,uint256 id,uint256 value) internal {
        require(from !=address(0),"from =0x00.. address");
        balanceOf[from][id] -=value;
        emit TransferSingle(msg.sender, from, address(0), id, value);
    }
  
    
    function _batchBurn(address from ,uint256[] calldata ids,
    uint256[] calldata values)
     internal {
        require(from !=address(0),"from =0x00.. address");
        require(ids.length==values.length,"ids length != values length");
...
contract MyMultiToken is ERC1155{
    ...
    function batchMint(uint256[] calldata ids, uint256[] calldata values,bytes calldata data)
    external{
